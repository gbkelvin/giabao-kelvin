...
import { FaFacebook, FaInstagram, 
        FaViber, FaEnvelope    }      from "react-icons/fa";
...
const Footer = () => {
    ...
    const onSendNewSletterEmail = async () => {
        ...
        if(validateInput(newsletterEmail) === false) {
            ...
            if(validationEmail(newsletterEmail) !== false) {
                ...
                FB_SERVICES.getNewSletterEmailByEmail(newsletterEmail)
                .then(
                    resultEmail => {
    ...
    useEffect(() => {
        FB_SERVICES.getCompanyInformation(languageTypeValue).then(footerInfo => setFooterInfo(footerInfo));
    }, [languageTypeValue]);
        ...
        return (
            <div className='newsletter-email__box'>
                ...
                <div className='newsletter__body'>
                    ...
                    {isEmailEmpty === true ? <div className='error__status'>{t("errorCheckInput.emailInputCheck")}</div> : null}
                    {isEmailFormat === false ? <div className='error__status'>{t("errorCheckInput.emailFormatCheck")}</div> : null}
